...
function provideLiquidityWithWidth(
    ...
    external {     
        ...
        IUniswapV3Pool public immutable pool = IUniswapV3Pool(poolAdress);
        ...
        uint P = преобразуем_к_нормальному_виду(sqrtPriceX96);

        uint k = (10 ** 4 - width) / (10 ** 4 + width);
        uint D = k * (amount1 - P * amount0) ** 2 - 4 * amount0 * amount1 * P * sqrt(k);

        uint p_a = (D - sqrt(k) * (amount1 - P * amount0)) ** 2 / (4 * (x ** 2) * P * k);
        uint p_b = p_a * k;
         ...
         nfpManager.mint(
            INonfungiblePositionManager.MintParams(
                ...
                address(token1),
                pool.fee(),
                преобразуй_в_тики(p_a), 
                преобразуй_в_тики(p_b), 
                amount0,
                amount1,
                0, // TODO: think about mev
                0, // TODO: think about mev
                msg.sender, // отправитель
                block.timestamp // дедлайн
            )
        );
}
